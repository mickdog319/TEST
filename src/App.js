...
function fetchStock(){
  ...
  let API_Call = `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${stock}&outputsize=compact&apikey=${API_KEY}`;
    fetch(API_Call)
    .then(
        function(response) {
  ...
  return (
    ...
    <>
      ...
      <p className='weather'>{weatherData.weather[0].main}</p></p>
